...
TELEGRAM_TOKEN = os.getenv('TELEGRimport time
import math
import requests
import pandas as pd
import os
...
def calculate_stoch_rsi(df):
    ...
    return stoch.rolling(K_SMOOTH).mean(), stoch.rolling(K_SMOOTH).mean().rolling(D_SMOOTH).mean()
...
# ====================== NUOVA FUNZIONE STARTUP ======================
def check_startup_signal():
    global bull_memory, bear_memory
    log("🔍 Controllo segnali recenti all'avvio...")
    df = get_market_data()
    if df is None: return

    # Controlliamo le ultime 3 candele chiuse (escludendo quella in corso)
    # df.iloc[-2] è l'ultima chiusa, -3 la precedente, -4 quella prima ancora
    for i in range(-4, -1):
        curr = df.iloc[i]
        prev = df.iloc[i-1]
    ...
    if side is None:
        if bull_memory > 0 and price > ema * 0.999:
            log("⚡ Segnale trovato all'avvio! Entro LONG.")
            place_trade("Buy", price)
            bull_memory = 0
        elif bear_memory > 0 and price < ema * 1.001:
            log("⚡ Segnale trovato all'avvio! Entro SHORT.")
            place_trade("Sell", price)
            bear_memory = 0
    log(f"Fine diagnosi avvio. BullMem: {bull_memory}, BearMem: {bear_memory}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            log(f"⏳ Prossimo check tra {int(wait)} secondi.")
            ...
            log(f"Prezzo:{price:.2f} | K:{curr['k']:.1f} D:{curr['d']:.1f} | Pos:{side}")

        except Exception as e:
            log(f"✗ Errore: {e}")
            time.sleep(30)AM_TOKEN')
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')
